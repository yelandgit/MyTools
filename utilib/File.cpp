...
static bool IsASCII(const strvec& text)
{
	for (strvec::const_iterator li=text.begin(); li!=text.end(); ++li)
		for (string::const_iterator i=(*li).begin(); i!=(*li).end(); ++i)
			if (WORD(*i)>128) return false;
	return true;
}

bool File::WriteText(LPCWSTR file, const vector<string>& text)
{
	File ff;
	if (!ff.Open(file, FILE_WRITE|FILE_TEXT)) return false;
	if (!IsASCII(text) && !ff.PutData("\xEF\xBB\xBF",3)) return false;
	for (strvec::const_iterator i=text.begin(); i!=text.end(); ++i)
	{
		string s = *i;
		while (s.size()>0 && (*s.rbegin()==' ' || *s.rbegin()=='\t')) s.pop_back();
		s += "\r\n"; if (!ff.PutData(s.c_str(),s.size())) return false;
	}
	ff.Close();
	return true;
}
...
size_t File::Size()
{
	...
	__int64 cpos = _ftelli64(m_file); _fseeki64(m_file, 0, SEEK_END);
	__int64 len = _ftelli64(m_file); _fseeki64(m_file, cpos, SEEK_SET);
	return (size_t)len;
}

size_t File::GetPosition()
{
	return size_t(m_file==nullptr || m_pipe ? 0 : _ftelli64(m_file));
}

size_t File::SetPosition(size_t pos)
{
	if (m_file==nullptr || m_pipe) return 0;
	size_t cpos = (size_t)_ftelli64(m_file); _fseeki64(m_file, pos, SEEK_SET);
