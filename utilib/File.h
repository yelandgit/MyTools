...
public:
	...
	bool Get(char& c);
	bool Get(wchar_t& wc);
	bool Get(short& d) { if (IsText()) return GetItem(d); return (IsOpenRead() ? fread(&d,sizeof(d),1,m_file)==1 : false); }
	bool Get(int& d) { if (IsText()) return GetItem(d); return (IsOpenRead() ? fread(&d,sizeof(d),1,m_file)==1 : false); }
	bool Get(long& d) { if (IsText()) return GetItem(d); return (IsOpenRead() ? fread(&d,sizeof(d),1,m_file)==1 : false); }
	bool Get(unsigned short& d) { if (IsText()) return GetItem(d); return (IsOpenRead() ? fread(&d,sizeof(d),1,m_file)==1 : false); }
	bool Get(unsigned int& d) { if (IsText()) return GetItem(d); return (IsOpenRead() ? fread(&d,sizeof(d),1,m_file)==1 : false); }
	bool Get(unsigned long& d) { if (IsText()) return GetItem(d); return (IsOpenRead() ? fread(&d,sizeof(d),1,m_file)==1 : false); }
	bool Get(float& d) { if (IsText()) return GetItem(d); return (IsOpenRead() ? fread(&d,sizeof(d),1,m_file)==1 : false); }
	bool Get(double& d) { if (IsText()) return GetItem(d); return (IsOpenRead() ? fread(&d,sizeof(d),1,m_file)==1 : false); }
	bool Get(long long& d) { if (IsText()) return GetItem(d); return (IsOpenRead() ? fread(&d,sizeof(d),1,m_file)==1 : false); }
	bool Get(unsigned long long& d) { if (IsText()) return GetItem(d); return (IsOpenRead() ? fread(&d,sizeof(d),1,m_file)==1 : false); }
	...
	static bool WriteText(LPCWSTR file, const vector<string>& text);
